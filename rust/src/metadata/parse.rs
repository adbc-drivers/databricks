...
/// Parsed primary key info from SHOW KEYS.
#[derive(Debug, Clone)]
...
/// Parsed foreign key info from SHOW FOREIGN KEYS.
#[derive(Debug, Clone)]
...
/// Parse primary keys from SHOW KEYS result.
///
/// Expected result columns from Databricks:
/// - `database_name` or `schema_name`: schema
/// - `table_name`: table
/// - `column_name`: column
/// - `key_seq` or `ordinal_position`: position (1-based)
/// - `constraint_name` or `pk_name`: constraint name
///
/// The exact column names may vary across Databricks versions.
/// We handle common variants.
pub fn parse_primary_keys(
    result: ExecuteResult,
    catalog: &str,
    schema: &str,
    table: &str,
    ...
    while let Some(batch) = reader.next_batch()? {
        ...
        // Try to find relevant columns by various known names
        let col_name_idx = batch
            .schema()
            .index_of("column_name")
            .or_else(|_| batch.schema().index_of("col_name"))
            .ok();

        let key_seq_idx = batch
            .schema()
            .index_of("key_seq")
            .or_else(|_| batch.schema().index_of("ordinal_position"))
            .ok();

        let pk_name_idx = batch
            .schema()
            .index_of("constraint_name")
            .or_else(|_| batch.schema().index_of("pk_name"))
            .ok();

        for row in 0..batch.num_rows() {
            let column_name = col_name_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_default();

            let key_seq = key_seq_idx
                .and_then(|i| get_optional_int32_value(&batch, i, row).ok())
                .flatten()
                .map(|v| v as i16)
                .unwrap_or((row as i16) + 1);

            let pk_name = pk_name_idx
                .and_then(|i| get_optional_string_value(&batch, i, row).ok())
                .flatten();
...
/// Parse foreign keys from SHOW FOREIGN KEYS result.
///
/// Expected result columns from Databricks:
/// - `fk_*` columns for foreign key table/column
/// - `pk_*` columns for primary key table/column
///
/// The exact column names may vary. We handle common variants.
pub fn parse_foreign_keys(
    result: ExecuteResult,
    catalog: &str,
    schema: &str,
    table: &str,
    ...
    while let Some(batch) = reader.next_batch()? {
        ...
        // Try to find FK columns
        let pk_catalog_idx = batch
            .schema()
            .index_of("pk_catalog_name")
            .or_else(|_| batch.schema().index_of("pktable_cat"))
            .ok();
        let pk_schema_idx = batch
            .schema()
            .index_of("pk_schema_name")
            .or_else(|_| batch.schema().index_of("pktable_schem"))
            .ok();
        let pk_table_idx = batch
            .schema()
            .index_of("pk_table_name")
            .or_else(|_| batch.schema().index_of("pktable_name"))
            .ok();
        let pk_column_idx = batch
            .schema()
            .index_of("pk_column_name")
            .or_else(|_| batch.schema().index_of("pkcolumn_name"))
            .ok();
        let fk_catalog_idx = batch
            .schema()
            .index_of("fk_catalog_name")
            .or_else(|_| batch.schema().index_of("fktable_cat"))
            .ok();
        let fk_schema_idx = batch
            .schema()
            .index_of("fk_schema_name")
            .or_else(|_| batch.schema().index_of("fktable_schem"))
            .ok();
        let fk_table_idx = batch
            .schema()
            .index_of("fk_table_name")
            .or_else(|_| batch.schema().index_of("fktable_name"))
            .ok();
        let fk_column_idx = batch
            .schema()
            .index_of("fk_column_name")
            .or_else(|_| batch.schema().index_of("fkcolumn_name"))
            .ok();
        let key_seq_idx = batch
            .schema()
            .index_of("key_seq")
            .or_else(|_| batch.schema().index_of("ordinal_position"))
            .ok();
        let fk_name_idx = batch
            .schema()
            .index_of("fk_constraint_name")
            .or_else(|_| batch.schema().index_of("fk_name"))
            .ok();
        let pk_name_idx = batch
            .schema()
            .index_of("pk_constraint_name")
            .or_else(|_| batch.schema().index_of("pk_name"))
            .ok();

        for row in 0..batch.num_rows() {
            let pk_catalog = pk_catalog_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_else(|| catalog.to_string());
            let pk_schema = pk_schema_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_else(|| schema.to_string());
            let pk_table = pk_table_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_else(|| table.to_string());
            let pk_column = pk_column_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_default();
            let fk_catalog = fk_catalog_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_else(|| catalog.to_string());
            let fk_schema = fk_schema_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_else(|| schema.to_string());
            let fk_table = fk_table_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_default();
            let fk_column = fk_column_idx
                .map(|i| get_string_value(&batch, i, row))
                .transpose()?
                .unwrap_or_default();
            let key_seq = key_seq_idx
...
mod tests {
    ...
    fn test_parse_primary_keys() {
        let schema = Arc::new(Schema::new(vec![
            ...
            Field::new("key_seq", DataType::Int32, false),
            Field::new("constraint_name", DataType::Utf8, true),
        ]));
        let batch = RecordBatch::try_new(
            schema,
            vec![
                Arc::new(StringArray::from(vec!["id", "tenant_id"])),
        ...
        let result = make_execute_result(vec![batch]);
        let keys = parse_primary_keys(result, "main", "default", "my_table").unwrap();

        assert_eq!(keys.len(), 2);
        assert_eq!(keys[0].catalog_name, "main");
        assert_eq!(keys[0].schema_name, "default");
        assert_eq!(keys[0].table_name, "my_table");
        assert_eq!(keys[0].column_name, "id");
        assert_eq!(keys[0].key_seq, 1);
        assert_eq!(keys[0].pk_name, Some("pk_my_table".to_string()));
        assert_eq!(keys[1].column_name, "tenant_id");
        assert_eq!(keys[1].key_seq, 2);
    }

    #[test]
    fn test_parse_primary_keys_empty() {
        let schema = Arc::new(Schema::new(vec![Field::new(
            "column_name",
            DataType::Utf8,
            false,
    ...
    fn test_parse_foreign_keys() {
        let schema = Arc::new(Schema::new(vec![
            ...
            Field::new("pk_schema_name", DataType::Utf8, false),
            ...
            Field::new("fk_schema_name", DataType::Utf8, false),
            Field::new("fk_table_name", DataType::Utf8, false),
            Field::new("fk_column_name", DataType::Utf8, false),
            Field::new("key_seq", DataType::Int32, false),
            Field::new("fk_constraint_name", DataType::Utf8, true),
            Field::new("pk_constraint_name", DataType::Utf8, true),
        ]));
        let batch = RecordBatch::try_new(
            schema,
            vec![
                Arc::new(StringArray::from(vec!["main"])),
                Arc::new(StringArray::from(vec!["default"])),
                Arc::new(StringArray::from(vec!["users"])),
                Arc::new(StringArray::from(vec!["id"])),
                Arc::new(StringArray::from(vec!["main"])),
                Arc::new(StringArray::from(vec!["default"])),
                Arc::new(StringArray::from(vec!["orders"])),
                Arc::new(StringArray::from(vec!["user_id"])),
                Arc::new(Int32Array::from(vec![1])),
                Arc::new(StringArray::from(vec![Some("fk_orders_users")])),
                Arc::new(StringArray::from(vec![Some("pk_users")])),
            ],
        )
        .unwrap();

        let result = make_execute_result(vec![batch]);
        let keys = parse_foreign_keys(result, "main", "default", "orders").unwrap();

        assert_eq!(keys.len(), 1);
        assert_eq!(keys[0].pk_catalog, "main");
        ...
        assert_eq!(keys[0].key_seq, 1);
        assert_eq!(keys[0].fk_name, Some("fk_orders_users".to_string()));
        assert_eq!(keys[0].pk_name, Some("pk_users".to_string()));
    }
}

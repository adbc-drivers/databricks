...
var drv = databricks.NewDriver(alloc)
...
const logSinkEnvVar = "ADBC_DRIVER_DATABRICKS_LOG_SINK"
...
func initLoggingFromEnv(db adbc.DatabaseLogging) {
	...
	case "tempfile":
		...
		f, err := os.CreateTemp("", "adbc-databricks.*.log")
		if err == nil {
			sink = f
		} else {
			fmt.Fprintf(os.Stderr, "[databricks] could not open temporary log file: %v", err)
		}
	}

...
### 2026-08-27: learnings since 2026-08-26T18:05:02Z
- **Context:** PR #645 fixed GetCrossReference over-return on the SEA path by filtering the SHOW FOREIGN KEYS rows against the caller's requested parent identifiers, mirroring the JDBC reference driver's CrossReferenceKeysDatabricksResultSetAdapter.includeRow.
  **Rule:** SHOW FOREIGN KEYS is scoped to the FOREIGN table only, so it returns FKs to EVERY parent — when the caller specifies parent catalog/schema/table you must filter the returned rows down to that parent client-side (case-insensitive equality; a null requested field means "no constraint" so the GetColumnsExtended foreign-only reuse passing null must stay unfiltered). Genuine FK rows always populate the parent identifier columns (JDBC's includeRow calls equalsIgnoreCase directly on them and would NPE otherwise), so treating a null/empty server parent value as a non-match is safe and correctly collapses the empty-string-parent case to zero rows.
- **Context:** PR #645 review (F1): the cross-reference parent filter initially compared against the value-population fallback (`srv ?? requested ?? ""`), so for any row whose server parent column was null it compared the requested value against itself — silently turning the filter into a no-op and re-admitting rows for the wrong parent.
  **Rule:** When a field is BOTH a filter key and populated with a fallback to the requested/input value, capture the raw source value FIRST and filter on that raw value; apply the requested-value fallback only afterward for row population. Filtering on the fallback compares the requested value against itself and disables the filter for exactly the rows where the source is absent/null.

### 2026-08-28: learnings since 2026-08-27T20:44:51Z
- **Context:** PR #656 moved the server feature-flag fetch from a background `Task.Run` to a synchronous blocking fetch on `Connect()`, initially serialized behind a single process-wide `SemaphoreSlim` on the `FeatureFlagCache` singleton; a reviewer caught the latency implication.
  **Rule:** When a per-connect network fetch is serialized behind a single process-wide lock, concurrent first-time (cold-cache) connects to DIFFERENT hosts queue behind one another and worst-case connect latency grows ~N×fetch-timeout — not the flat single-timeout a "bounded by the fetch timeout" comment implies. Key the create/dedup lock per-host (e.g. `ConcurrentDictionary<host, SemaphoreSlim>`) so different cold hosts fetch in parallel, while same-host concurrent connects still dedup to one fetch via a post-lock double-check.
- **Context:** PR #656 added a lazily `GetOrAdd`'d per-host `SemaphoreSlim` map on the process-lifetime `FeatureFlagCache` singleton; a reviewer flagged it had no removal path (leaking one lock per host talked to), fixed by removing the lock in the cache eviction callback.
  **Rule:** A per-key lock map kept alongside a cache must drop its entry in the cache's eviction callback (not only in `Dispose`), or it grows unbounded over a long-lived process talking to many keys. On eviction, `TryRemove` the lock reference rather than `Dispose()`-ing it: an in-flight holder may still be inside `WaitAsync`/`Release`, and disposing out from under them throws `ObjectDisposedException`. A `SemaphoreSlim` whose `AvailableWaitHandle` was never accessed holds no OS handle, so dropping the reference leaks nothing; a racing create may briefly re-add a fresh lock, weakening dedup for one window but never correctness.
- **Context:** PR #654 wrapped the Thrift `GetTables` result stream (`TableTypeDefaultingStream`) to substitute `"TABLE"` for any null/empty `TABLE_TYPE`, matching databricks-jdbc `MetadataResultSetBuilder`.
  **Rule:** `GetTables` must not return rows with an empty/null `TABLE_TYPE` — the Thrift path leaves some tables (e.g. legacy `hive_metastore` tables in a broad enumeration) with an empty type, so default any empty/null server-supplied `TABLE_TYPE` to `"TABLE"` for JDBC parity (bluntly, like databricks-jdbc: even a view with an empty server type becomes `"TABLE"`; non-empty values pass through). Implement by wrapping the result stream and rewriting only the plain `StringArray` encoding (fail-open / no-op on other encodings), and guard a null `result.Stream` and pass it through, matching the sibling `MaybeWrapComplexTypes` wrapper.

### 2026-09-01: learnings since 2026-08-31T17:26:42Z
- **Context:** PR #660 (csharp SEA/StatementExecution GetCrossReference) — `_metadataCatalogName` is seeded from the connection's default catalog at construction, so passing it as the parent-catalog filter in a getImportedKeys-style call (no parent supplied) wrongly dropped every FK whose real parent lives in another catalog; SEA returned empty while Thrift returned the row.
  **Rule:** Fields seeded from the connection's default catalog (e.g. `_metadataCatalogName`) are NOT a reliable "caller supplied this argument" signal — track an explicit "was set" flag (set only in SetOption/CatalogName) and pass the parent catalog into cross-reference/metadata filters only when the caller explicitly set it, else null; unseeded fields like parent schema/table already no-op when unset. Regression fixtures must use a parent catalog DIFFERENT from the seeded default (e.g. seed "main", FK parent "prod_cat"), because a fixture whose parent catalog equals the seed makes the seeded value happen to match and masks the bug.

### 2026-09-04: learnings since 2026-09-03T17:28:33Z
- **Context:** PR #662 de-flaked C# E2E MAP-column assertions (`ComplexTypesValueTests` / `DatabricksTestEnvironment.NormalizeMapJson`) that had compared serialized MAPs by exact key order.
  **Rule:** Databricks `MAP` key order is unspecified server-side and the JDBC reference preserves that server order without sorting, so compare a serialized MAP's content order-insensitively — normalize by sorting only the top-level object's keys. Keep the normalization SHALLOW: preserve each value's raw JSON verbatim so nested STRUCT field order (which IS significant) is untouched; a MAP nested inside a value re-introduces the key-order flake and would need recursive normalization.
- **Context:** PR #509 rewrote `ComplexTypeSerializingStream` to a manual `Utf8JsonWriter` for AOT/trim safety; a reviewer caught that `WriteRawValue` was used for Time32/Time64 values formatted as `HH:mm:ss.ffffff`.
  **Rule:** `Utf8JsonWriter.WriteRawValue` requires a syntactically valid JSON token (number/string/bool/null/object/array) and throws `JsonException` at runtime for anything else — an unquoted time/date string with colons is NOT valid JSON. Use `WriteStringValue` for values that must be quoted strings and reserve `WriteRawValue` for raw numeric text (e.g. full-precision DECIMAL).
- **Context:** PR #509 replaced reflection-based `JsonSerializer.Serialize(object)` with a hand-written explicit type-switch (`SerializeStructuredValue`) and a hand-written telemetry payload writer, to make the driver trim-/NativeAOT-safe under net10.0 (`IsAotCompatible`).
  **Rule:** Replacing a reflection serializer with a hand-written one silently changes semantics: the switch's `default` arm mis-categorizes any unenumerated type (e.g. emitting a number as a JSON string), and a new property on a model is silently omitted with no test failure — behaviors the reflection path handled automatically. Defend the rewrite with a round-trip test asserting hand-written output equals the reflection-based output for a fully-populated instance (keep the reflection options around for tests), and/or a `Debug.Assert(false, ...)` in the default arm so DEBUG unit tests catch newly-added types before they ship.

### 2026-09-09: learnings since 2026-09-08T17:27:54Z
- **Context:** PR #670 added a process-wide `static` ReydenWarehouseCache; a reviewer caught it was keyed on the bare warehouse id.
  **Rule:** Databricks SQL warehouse ids are only workspace-unique (small zero-padded counters, not globally-unique GUIDs), so any process-wide/static per-warehouse cache MUST be keyed on a `(host, warehouseId)` composite — a bare-id key lets one workspace's mark leak to a same-id warehouse in another workspace (silent protocol/behavior switch for the TTL). Lowercase the host component to match sibling caches (FeatureFlagCache) so mixed-casing hosts don't split into distinct entries.
- **Context:** PR #670's `Connect` constructs a connection then calls `OpenAsync().Wait()` / `ApplyServerSidePropertiesAsync().Wait()`; a reviewer flagged that a partial open (OpenAsync succeeds, ApplyServerSideProperties throws) leaks the connection's HTTP client / handler chain — worse now that a fallback opens a second connection.
  **Rule:** When a factory constructs-then-opens a resource that owns an HTTP client / handler chain (especially when a fallback may open another), wrap the open in `try { open(); return conn; } catch { conn.Dispose(); throw; }` so a partial or failed open disposes it before propagating — and apply the dispose-on-failure guard symmetrically to EVERY open path, not just the one that triggers the fallback.
- **Context:** PR #670's transparent Thrift→SEA fallback originally discarded the original Thrift rejection when the SEA fallback also threw, leaving it recoverable only from an untraced Activity event.
  **Rule:** When a transparent fallback (e.g. a protocol downgrade) also fails, throw an AggregateException retaining BOTH the original error and the fallback error so the causal chain survives outside of tracing — never let the primary attempt's exception be silently dropped. Order the inner exceptions deliberately so the existing AggregateException→AdbcException unwrap (a depth-first search for the first AdbcException) surfaces the intended primary.
- **Context:** PR #670's transparent Thrift→SEA fallback initially emitted no telemetry, so an operator who set `protocol=thrift` and observed a SEA connection had no signal why the protocol changed.
  **Rule:** A transparent/silent protocol or behavior downgrade must emit a trace/telemetry event (matching the codebase's existing `Activity.Current?.AddEvent(...)` convention) at BOTH the pre-check short-circuit and the catch-and-retry, tagged with the identifying key, or the switch is undiagnosable in the field. Emit discrete tag dimensions (e.g. host, warehouse_id) rather than a composite key that embeds a separator like '\n' — some OpenTelemetry exporters/log sinks mangle or truncate embedded newlines/control chars.
- **Context:** PR #670 made the driver transparently fall back to SEA when a Reyden / Lakehouse-RT warehouse rejects a Thrift `OpenSession`.
  **Rule:** Reyden / Lakehouse-RT SQL warehouses do not support Thrift and reject `OpenSession` with SQLSTATE `KP001` (surfaced on `HiveServer2Exception.SqlState`); detect it by walking the whole exception chain INCLUDING `AggregateException.InnerExceptions`, and cache the rejection per `(host, warehouseId)` with a bounded self-healing TTL so later connects skip the doomed handshake — the Thrift `OpenSession` otherwise retries across protocol versions (V10→V7) before it fails.

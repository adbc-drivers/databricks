...
namespace AdbcDrivers.Databricks.Tests.Unit.Telemetry
{
    ...
    public class DatabricksTelemetryExporterTests
    {
        ...
        [Fact]
        public async Task DatabricksTelemetryExporter_ExportAsync_EmptyList_ReturnsImmediately()
        {
            // Arrange
            var requestCount = 0;
            var handler = new MockHttpMessageHandler((request, ct) =>
        ...
        [Fact]
        public async Task DatabricksTelemetryExporter_ExportAsync_NullList_ReturnsImmediately()
        {
            // Arrange
            var requestCount = 0;
            var handler = new MockHttpMessageHandler((request, ct) =>
        ...
        public async Task DatabricksTelemetryExporter_ExportAsync_MaxRetries_DoesNotThrow()
        {
            ...
            // Act & Assert - should not throw even after all retries exhausted
            await exporter.ExportAsync(logs);

            // Assert - should have tried initial attempt + max retries
            Assert.Equal(4, attemptCount); // 1 initial + 3 retries
        }

        [Fact]
        public async Task DatabricksTelemetryExporter_ExportAsync_TerminalError_DoesNotRetry()
        {
            // Arrange
            var attemptCount = 0;
            var handler = new MockHttpMessageHandler((request, ct) =>
            ...
            // Assert - should not retry on terminal error
        ...
        [Fact]
        public async Task DatabricksTelemetryExporter_ExportAsync_GenericException_DoesNotThrow()
        {
            // Arrange
            var handler = new MockHttpMessageHandler((request, ct) =>
            {

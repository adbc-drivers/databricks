...
namespace AdbcDrivers.Databricks.Tests.E2E.Telemetry
{
    ...
    public class ClientTelemetryE2ETests : TestBase<DatabricksTestConfiguration, DatabricksTestEnvironment>
    {
        ...
        public async Task CanSendTelemetryToAuthenticatedEndpoint()
        {
            ...
            // Send telemetry - should not throw
            var exception = await Record.ExceptionAsync(() => exporter.ExportAsync(logs));

            // ExportAsync should not throw exceptions (per design requirement)
        ...
        public async Task EmptyLogListDoesNotSendRequest()
        {
            ...
            // Send empty list - should return immediately without error
            var exception = await Record.ExceptionAsync(() => exporter.ExportAsync(new List<TelemetryFrontendLog>()));

            Assert.Null(exception);
            OutputHelper?.WriteLine("Empty log list handled gracefully");

            // Send null list - should also return immediately without error
            exception = await Record.ExceptionAsync(() => exporter.ExportAsync(null!));
        ...
        public async Task TelemetryExporterRespectsRetryConfiguration()
        {
            ...
            // This should succeed (or gracefully fail after retries)
            var exception = await Record.ExceptionAsync(() => exporter.ExportAsync(logs));

            // Exporter should never throw per design
            Assert.Null(exception);

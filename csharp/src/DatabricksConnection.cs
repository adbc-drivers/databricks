    ...
    internal class DatabricksConnection : SparkHttpConnection
    {
        ...
        private async Task InitializeTelemetryAsync(Activity? activity = null)
        {
            try
            {
                ...
                bool serverFeatureFlagEnabled = featureFlagContext.IsFeatureEnabled(TelemetryConfiguration.FeatureFlagName);
        ...
        private void ReleaseTelemetryResources()
        {
            ...
            try
            {
                // Release feature flag context (decrements ref count, removes if last)
                var featureFlagCache = FeatureFlagCache.GetInstance();
                featureFlagCache.ReleaseContext(_telemetryHost);

                Debug.WriteLine($"[TRACE] Telemetry: Released feature flag context for host '{_telemetryHost}'");
            }
            catch (Exception ex)
            {
                // Swallow all exceptions per telemetry requirement
                Debug.WriteLine($"[TRACE] Telemetry: Error releasing feature flag context: {ex.Message}");
            }
